...
```text
find the number of numbers in the range of A, B that can be divided by d and leave a remainder of r. 
```
...
```text
take input of A, B d and r. create an empty list N = []. use for loop in range of A to B. run the numbers through the modulo and if it equals r. add it to the N list. print the length of N
```
...
```text
normal test case with no remainder
```
